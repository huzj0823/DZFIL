...
/* Write to PR_HLS Address offset registers to set the location in Memory where Input Data and Output results are stored */
    fpga_AXI_Lite_ptr->fpga_poke(PR_HLS_NORTH_INPUT_BYTES_L, (uint32_t)(0x00000000FFFFFFFFULL & NUM_OF_DATA_SETS));
    fpga_AXI_Lite_ptr->fpga_poke(PR_HLS_NORTH_INPUT_BYTES_H, (uint32_t)((0xFFFFFFFF00000000ULL & NUM_OF_DATA_SETS) >> 32));
    fpga_AXI_Lite_ptr->fpga_poke(PR_HLS_NORTH_CONTROL_REG, 0x1); // reset statistics
    fpga_AXI_Lite_ptr->fpga_poke(PR_HLS_NORTH_CONTROL_REG, 0x0); // release
}

uint64_t  fpga_check_completion_bytes_NORTH_PR (fpga_xDMA_linux *fpga_AXI_Lite_ptr) {
    uint64_t output_size = 0;
    output_size = (uint64_t)fpga_AXI_Lite_ptr->fpga_peek(PR_HLS_NORTH_OUTPUT_BYTES_L) | ((uint64_t)fpga_AXI_Lite_ptr->fpga_peek(PR_HLS_NORTH_OUTPUT_BYTES_H) << 32);
    return output_size;
}

int  fpga_check_compute_done_NORTH_PR (fpga_xDMA_linux *fpga_AXI_Lite_ptr) {
    return fpga_AXI_Lite_ptr->fpga_peek(PR_HLS_NORTH_STATUS_REG) & 0x1;
}
